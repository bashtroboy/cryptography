## About Network Fundamentals Course
The existence of a network can be discussed not only in the field of cyber security but in almost every structure where digital devices are located. Networks created by digital devices are in the use of people from every profession today. Some people often use devices connected to the network, unaware of how network technologies work. The situation is different for experts in the field of cyber security. Technical employees in the cyber security industry should have basic knowledge of networking, regardless of their area of ​​expertise. It is very important to ensure the security of networks connected to digital devices in every corner of our lives. It is not possible to consider a device independent of the network. Since the devices are connected to the network, they are in communication with other devices in the same network. In this case, the most basic requirement for network-based attacks is fulfilled. SOC analysts depend heavily on their networking know-how to be able to detect threats and protect the devices/networks. To be able to do it, they must be aware of the structure of networks so that they can detect the threats.

Networking is a very comprehensive subject and the first part of this training series covers basic networking issues. At the end of the training, it is recommended that the candidates who will master the basic topics about networking complete the topics in this training without skipping.
